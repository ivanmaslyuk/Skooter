...
class Text(View):
    ...
    def paint(self, canvas: skia.Canvas, x: float, y: float, width: float, height: float):
        ...
        text_width = font.measureText(str(self.__text), skia.TextEncoding.kUTF8)
        ...
        canvas.drawString(str(self.__text), x, y + line_height - metrics.fBottom, font, paint)

        self.draw_children(canvas, x, y, width, height)

    def get_bounding_rect(self) -> Rect:
        font = skia.Font(None, self.__size)
        width = font.measureText(str(self.__text), skia.TextEncoding.kUTF8)
        metrics = font.getMetrics()
        line_height = abs(metrics.fTop) + abs(metrics.fBottom)
